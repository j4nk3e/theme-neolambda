function fish_prompt
  ...
  if not set -q __fish_prompt_hostname
    ...
    set -g __fish_prompt_hostname (set_color orange)(hostname|cut -d . -f 1)(set_color normal)
  end
  ...
  if [ "$SSH_TTY" = "" ]
    ...
    if [ "$TERM" = "xterm-256color-italic" -o "$TERM" = "tmux-256color" ]
      ...
      set -g location ssh
      # set -g ssh_hostname (echo -e $blue$__fish_prompt_hostname)
      set -g __fish_prompt_hostname (set color blue)(hostname|cut -d . -f 1)(set_color normal)
    end
  end

  if [ (id -u) -eq 0 ]
    # top line > Superuser
    echo -n $red'╭─'$hotpink$USER $location $__fish_prompt_hostname$white' in '$limegreen(pwd)$turquoise
    __fish_git_prompt " (%s)"
    echo
